...
void ACppHubActor::BeginPlay()
{
	...
	GetWorldTimerManager().SetTimer(
		SpawnTimerHandle, this, &ACppHubActor::OnTimeToSpawn, SpawnTimerRate, true);
}
...
void ACppHubActor::OnTimeToSpawn()
{
	if (++CurrentTimerCount <= TotalCount)
	{
		const FVector Location = GetActorLocation() + 
			FVector(FMath::FRandRange(200.0f, 1000.0f), FMath::FRandRange(200.0f, 1000.0f), 0.0f);
	...
	else
	{
		...
		{
			GetWorldTimerManager().SetTimer(
				DestroyTimerHandle, this, &ACppHubActor::OnTimeToDestroy, DestroyTimerRate, true);
		}
	}
}
